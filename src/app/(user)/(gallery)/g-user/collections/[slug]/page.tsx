...
// commented this code but it's working, did it for review of the design
// don't delete this
...
async function getCollection(slug: string) {
  // Fetch the collection from the 'child_collection' table based on the slug
  const { data, error } = await supabase
    .from('child_collection')
    .select('*')
    .eq('sluger', slug); // Assuming you filter by 'sluger' field
  ...
  // Return the first collection if found
  if (data && data.length > 0) {
    const collection = data[0];

    // Assuming 'path' contains multiple image URLs, extract them into an array
    const images = data.map(item => item.path); // Create an array with image URLs

    return {
      title: collection.title,
      description: collection.desc,
      images: images, // Use the populated images array
...
type CollectionType = {
  title: string;
  description: string;
  images: string[]; // Now populated from the database
  artist: string;
  year: string;
};
  ...
  return (
    <div className="bg-white min-h-screen pt-32">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
          {collection.images.map((img: string, index: number) => (
            <motion.div 
              key={index}
              initial={{ opacity: 0, scale: 0.9 }}
              animate={{ opacity: 1, scale: 1 }}
